...
fn count_all_combinations(containers: Vec<usize>, max: usize) -> usize {
    fn build(containers: &[usize], max: usize, mut v: Vec<Vec<usize>>, ans: &mut usize) {
        ...
        if !new_v.is_empty() {
            build(containers, max, new_v, ans)
        }
    }
    ...
    build(&containers, max, v, &mut ans);
    ans
}
...
mod tests {
    ...
    #[test]
    fn test_process() {
        assert_eq!(count_all_combinations(vec![20, 15, 10, 5, 5], 25), 4);
    }
}
