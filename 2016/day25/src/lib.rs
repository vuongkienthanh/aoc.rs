        ...
        for (i, v) in self.program.windows(5).enumerate() {
            match (v[0], v[1], v[2], v[3], v[4]) {
                (
                    Item::inc(r1),
                    Item::dec(r2),
                    Item::jnz(Target::Register(r3), Target::Value(-2)),
                    Item::dec(r4),
                    Item::jnz(Target::Register(r5), Target::Value(-5)),
                ) if r2 == r3 && r4 == r5 => {
                ...
                (
                    Item::jnz(Target::Register(r1)),
                    _,
                    Item::dec(r2),
                    Item::dec(r3),
                    Item::jnz(Target::Register(r4), Target::Value(-4)),
                ) if r1 == r2 && r3 == r4 => {}
                _ => (),
            }
        }
...
impl Item {
    fn run(self, computer: &mut Computer) -> Option<isize> {
        match self {
            Item::cpy(t, r) => {
                match t {
            ...
            Item::out(r) => {
                computer.i += 1;
                return Some(computer.registers[r]);
            }
