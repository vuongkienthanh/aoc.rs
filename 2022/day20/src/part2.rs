...
pub fn process(_input: &str) -> isize {
    let input: Vec<_> = parse_input(_input)
        .into_iter()
        .map(|v| v * 811589153)
        .collect();
    let tlen = input.len() as isize - 1;
    let optimized_input: Vec<_> = input
        .iter()
        .map(|v| {
            [(v % tlen + tlen) % tlen, (v % tlen + tlen) % tlen - tlen]
                .into_iter()
                .min_by_key(|x| x.abs())
                .unwrap()
        })
    ...
    for _ in 0..10 {
        for (i, v) in optimized_input.iter().enumerate() {
            if *v == 0 {
                continue;
            }
...
4"#
    ...
    fn test_process_(fixture: &str) {
        assert_eq!(process(fixture), 1623178306);
    }
}
