...
impl Computer {
    ...
    pub fn append_input(&mut self, input: i128) {
        self.input.push_back(input);
    }

    pub fn get_value(&self, modes: &mut i128, parameter_pointer: usize) -> i128 {
        let mode = *modes % 10;
        (*modes) /= 10;
        match mode {
            0 => {
    ...
    pub fn get_loc(&self, modes: &mut i128, parameter_pointer: usize) -> usize {
        let mode = *modes % 10;
        (*modes) /= 10;
        match mode {
            0 => self
    ...
    pub fn run(&mut self) -> RunResult {
        ...
        match Opcode::from(opcode) {
            Opcode::add => {
                ...
                self.prog.insert(c as usize, a + b);
                self.pointer += 4;
            }
            Opcode::mul => {
                let a = self.get_value(&mut modes, self.pointer + 1);
            ...
            Opcode::lt => {
                ...
                let c = self.get_loc(&mut modes, self.pointer + 3);
                if a < b {
                    self.prog.insert(c as usize, 1);
                } else {
                    self.prog.insert(c as usize, 0);
                }
                self.pointer += 4;
            }
            Opcode::eq => {
    ...
    pub fn long_run(&mut self) -> Option<i128> {
        loop {
            match self.run() {
                RunResult::Running => (),
