...
fn create_matrix(buttons: Vec<Vec<usize>>, target: Vec<usize>) -> Matrix {
...
fn expand(matrix: &mut Matrix, btn_len: usize) {
    ...
    for comb in matrix.iter().combinations(3) {
        ...
        'case: for case in [[0, 1, 2], [0, 2, 1], [1, 2, 0]] {
            ...
            let mut row_a = vec![0isize; btn_len];
            let mut row_b = vec![0isize; btn_len];
            let mut row_c = vec![0isize; btn_len];
            ...
            for col in 0..btn_len {
                result_row[col] = row_a[col] + row_b[col] - row_c[col];
