...
pub fn process(_input: &str) -> usize {
    ...
    for (turn, ((x0, x1), (y0, y1), (z0, z1))) in input {
        let (x0, x1, y0, y1, z0, z1) =
            if x0 > 50 || x1 < -50 || y0 > 50 || y1 < -50 || z0 > 50 || z1 < -50 {
                ...
                (
                    (x0 + 50).max(0) as usize,
                    (x1 + 50).min(100) as usize,
                    (y0 + 50).max(0) as usize,
                    (y1 + 50).min(100) as usize,
                    (z0 + 50).max(0) as usize,
    ...
    space.into_iter().flatten().flatten().filter(|x| *x).count()
}
#[cfg(test)]
mod tests {
    use super::*;
...
on x=967..23432,y=45373..81175,z=27513..53682"#
    ...
    fn test_process_(fixture: &str) {
        assert_eq!(process(fixture), 590784);
    }
}
